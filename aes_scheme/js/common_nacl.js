...
var common = (function() {
  ...
  function createNaClModule(name, tool, path, width, height, attrs) {
    ...
    moduleEl.setAttribute('src', prefix_path + path + '/' + name + '.nmf');
  ...
  var defaultMessageTypes = {
    ...
    'log': logMessage,
    'encryption': doUpload,
    'decryption': completeDownloadFile
  };
  ...
  function handleMessage(message_event) {
    ...
    {
      if (message_event.data.event.toString() === 'encryption'){
        doUpload(message_event.data);
        return;
      }
      else if (message_event.data.event.toString() === 'decryption'){
        completeDownloadFile(message_event.data);
        return;
      }
      ...
      else if (message_event.data.event.toString() === 'updating'){
        completeUpdate(message_event.data);
        return;
      }
    }
